...
/*
 * BLE utitilies, etc. from Adafruit library.
 */
public class BleUtils {
    ...
    // Use this check to determine whether BLE is supported 0on the device.  Then you can  selectively disable BLE-related features.
    ...
    public static void resetBluetoothAdapter(Context context, ResetBluetoothAdapterListener listener) {
        if (sResetHelper == null) {
            ...
            Log.w(AppConstants.LOG_TAG, "Reset already in progress");
        }
    }
    ...
    private static class ResetBluetoothAdapter {
        ...
        ResetBluetoothAdapter(Context context, ResetBluetoothAdapterListener listener) {
            ...
            if (bleAdapter.isEnabled()) {
                ...
                if (isDisablingBle) {
                    Log.w(AppConstants.LOG_TAG, "Reset ble adapter started. Waiting to turn off");
                    // Now wait for BluetoothAdapter.ACTION_STATE_CHANGED notification
                } else {
                    Log.w(AppConstants.LOG_TAG, "Can't disable bluetooth adapter");
                    resetCompleted();
                }
            }
        }
        ...
        private void onBleAdapterStatusChanged(int state) {
            switch (state) {
                case BluetoothAdapter.STATE_OFF: {
                    ...
                    Log.d(AppConstants.LOG_TAG, "Ble adapter turned off. Turning on");
                    BluetoothAdapter bleAdapter = BleUtils.getBluetoothAdapter(mContext);
                    bleAdapter.enable();
                    break;
                }
                ...
                case BluetoothAdapter.STATE_ON: {
                    Log.d(AppConstants.LOG_TAG, "Ble adapter turned on. Reset completed");
    ...
    public static UUID getUuidFromByteArrayLittleEndian(byte[] bytes) {
