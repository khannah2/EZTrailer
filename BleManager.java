...
public class BleManager implements BleGattExecutor.BleExecutorListener {
    ...
    public BleManager(Context context) {
        ...
        if (mAdapter == null || !mAdapter.isEnabled()) {
            Log.e(AppConstants.LOG_TAG, "Unable to obtain a BluetoothAdapter.");
        }
    }
    ...
    public boolean connect(Context context, String address) {
        if (mAdapter == null || address == null) {
            Log.w(AppConstants.LOG_TAG, "connect: BluetoothAdapter not initialized or unspecified address.");
            return false;
        }
        ...
        if (reuseExistingConnection) {
            ...
            if (mDeviceAddress != null && address.equalsIgnoreCase(mDeviceAddress) && mGatt != null) {
                Log.d(AppConstants.LOG_TAG, "Trying to use an existing BluetoothGatt for connection.");
                if (mGatt.connect()) {
        ...
        if (mDevice == null) {
            Log.w(AppConstants.LOG_TAG, "Device not found.  Unable to connect.");
            return false;
        }
...
*/
        ...
        Log.d(AppConstants.LOG_TAG, "Trying to create a new connection.");
    ...
    public boolean refreshDeviceCache(){
        try {
            ...
            if (localMethod != null) {
                ...
                if (result) {
                    Log.d(AppConstants.LOG_TAG, "Bluetooth refresh cache");
                }
                return result;
            }
        }
        catch (Exception localException) {
            Log.e(AppConstants.LOG_TAG, "An exception occurred while refreshing device");
        }
        return false;
    }
    ...
    public void disconnect() {
        ...
        if (mAdapter == null || mGatt == null) {
            Log.w(AppConstants.LOG_TAG, "disconnect: BluetoothAdapter not initialized");
            return;
        }
/*
...
*/
    ...
    private void readService(BluetoothGattService service, String characteristicUUID, String descriptorUUID) {
        if (service != null) {
            if (mAdapter == null || mGatt == null) {
                Log.w(AppConstants.LOG_TAG, "readService: BluetoothAdapter not initialized");
                return;
            }
    ...
    public void writeService(BluetoothGattService service, String uuid, byte[] value)
    {
        if (service != null) {
            if (mAdapter == null || mGatt == null) {
                Log.w(AppConstants.LOG_TAG, "writeService: BluetoothAdapter not initialized");
                return;
            }
    ...
    public void enableNotification(BluetoothGattService service, String uuid, boolean enabled) {
        if (service != null) {
            ...
            if (mAdapter == null || mGatt == null) {
                Log.w(AppConstants.LOG_TAG, "enableNotification: BluetoothAdapter not initialized");
                return;
            }
    ...
    public void onServicesDiscovered(BluetoothGatt gatt, int status) {
        ...
        if (status != BluetoothGatt.GATT_SUCCESS) {
            Log.d(AppConstants.LOG_TAG, "onServicesDiscovered status: " + status);
        }
    }
    ...
    public void onCharacteristicRead(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
        ...
        if (status != BluetoothGatt.GATT_SUCCESS) {
            Log.d(AppConstants.LOG_TAG, "onCharacteristicRead status: "+status);
        }
    }
    ...
    public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
        // if (status == BluetoothGatt.GATT_SUCCESS) {
        if (mBleListener != null) {
            mBleListener.onDataWritten(characteristic);
        }
        // }

        if (status != BluetoothGatt.GATT_SUCCESS) {
            Log.d(AppConstants.LOG_TAG, "onCharacteristicWrite status: "+status);
        }
    }
    ...
    public void onDescriptorRead(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
        ...
        if (status != BluetoothGatt.GATT_SUCCESS) {
            Log.d(AppConstants.LOG_TAG, "onDescriptorRead status: "+status);
        }
    }
    ...
    public void onReadRemoteRssi(BluetoothGatt gatt, int rssi, int status) {
        ...
        if (status != BluetoothGatt.GATT_SUCCESS) {
            Log.d(AppConstants.LOG_TAG, "onReadRemoteRssi status: "+status);
        }
